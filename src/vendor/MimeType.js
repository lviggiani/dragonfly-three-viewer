...
export default class MimeType {
    ...
    static catalog = {};
    static lookup(fname, include_charset, default_mime_type) {
	    let ext, charset = this.charset;
	...
	static set(exts, mime_type_string) {
	    let result = true;
        //console.log("DEBUG exts.indexOf(',')", typeof exts.indexOf(','), exts.indexOf(','));
	    if (exts.indexOf(',') > -1) {
		    exts.split(',').forEach(ext => {
			    ext = ext.trim();
			    this.catalog[ext] = mime_type_string;
			    if (this.catalog[ext] !== mime_type_string) {
				    result = false;
			    }
		    });
	    } else {
		    this.catalog[exts] = mime_type_string;
	    }
	    return result;
    }
    ...
    static forEach(callback) {
	    ...
	    for (ext in this.catalog) {
		    // deno-lint-ignore no-prototype-builtins
		    if (this.catalog.hasOwnProperty(ext)) {
			    callback(ext, this.catalog[ext]);
		    }
	    }
	    return this.catalog;
    }
...
MimeType.set(".dae", "model/vnd.collada+xml");
MimeType.set(".glb", "model/gltf-binary")
...
// Custom
MimeType.set(".lz", "application/x-lzip");
MimeType.set(".lzma", "application/x-lzma");
MimeType.set(".hdr", "image/vnd.radiance");
MimeType.set(".exr", "image/x-exr");